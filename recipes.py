...
def Rezept_eintragen(w, DB, c):
    ...
    if (val_check == 0 and str_check == 0 and double_check ==  0):
        c.execute("SELECT COUNT(*) FROM Rezepte WHERE Name=?",(w.LECocktail.text(),))
        ...
        if Rezepttest == 0:
            ...
            for Anzahl in range(0, len(Zutaten_V)):
                temp1 = c.execute("SELECT Alkoholgehalt FROM Zutaten WHERE Name = ?",(Zutaten_V[Anzahl],))
                for temp3 in temp1:
            ...
            c.execute("INSERT OR IGNORE INTO Rezepte(Name, Alkoholgehalt, Menge, Kommentar, Anzahl_Lifetime, Anzahl) VALUES (?,?,?,?,0,0)",(w.LECocktail.text(),Alkoholgehalt_Cocktail,SVol,w.LEKommentar.text()))
            #RezeptID, sowie ZutatenIDs werden herausgesucht und anschließend in die Zusammen DB eingetragen
            for Anzahl in range(0, len(Zutaten_V)):
                temp1 = c.execute("SELECT ID FROM Zutaten WHERE Name = ?",(Zutaten_V[Anzahl],))
                for temp3 in temp1:
                    ZutatenDBID = temp3[0]
                ...
                for temp3 in temp1:
                    if temp3[0] > 0:
                        ...
                        isalkoholic = 0
                temp2 = c.execute("SELECT ID FROM Rezepte WHERE Name = ?",(w.LECocktail.text(),))
                for temp3 in temp2:
                    RezepteDBID = temp3[0]
                print(RezepteDBID, ZutatenDBID, Mengen_V[Anzahl], isalkoholic)
                c.execute("INSERT OR IGNORE INTO Zusammen(Rezept_ID, Zutaten_ID, Menge, Alkoholisch) VALUES (?, ?, ?, ?)",(RezepteDBID, ZutatenDBID, Mengen_V[Anzahl], isalkoholic))
            DB.commit()
...
def Rezepte_Rezepte_click(w, DB, c):
    ...
    c.execute("SELECT Kommentar FROM Rezepte WHERE Name = ?",(w.LWRezepte.currentItem().text(),))
...
def Rezept_aktualisieren(w, DB, c):
    ...
    Zspeicher = c.execute("SELECT ID FROM Rezepte WHERE Name = ?",(altername,))
    for row in Zspeicher:
        CocktailID = row[0]
    ...
    if (val_check == 0 and str_check == 0 and double_check ==  0):
        ...
        if neuerName != "":
            c.execute("UPDATE OR IGNORE Rezepte SET Name = ? WHERE ID = ?",(neuerName,int(CocktailID)))
        ...
        #print(Alkoholgehalt_Cocktail)
        #Rezept wird in Rezept DB eingetragen
        c.execute("UPDATE OR IGNORE Rezepte SET Alkoholgehalt = ?, Menge = ?, Kommentar = ? WHERE ID = ?",(Alkoholgehalt_Cocktail,SVol,w.LEKommentar.text(),int(CocktailID)))
        #RezeptID, sowie ZutatenIDs werden herausgesucht und anschließend in die Zusammen DB eingetragen
        for Anzahl in range(0, len(Zutaten_V)):
            temp1 = c.execute("SELECT ID FROM Zutaten WHERE Name = ?",(Zutaten_V[Anzahl],))
            for temp3 in temp1:
            ...
            for temp3 in temp1:
                if temp3[0] > 0:
                    ...
                    isalkoholic = 0
            c.execute("INSERT OR IGNORE INTO Zusammen(Rezept_ID,Zutaten_ID,Menge,Alkoholisch) VALUES (?,?,?,?)",(int(CocktailID),ZutatenDBID,Mengen_V[Anzahl],isalkoholic))
        DB.commit()
...
def Rezepte_delete(w, DB, c):
    ...
    if w.LEpw.text() == globals.masterpassword:
        if not w.LWRezepte.selectedItems():
            ...
            Zspeicher = c.execute("SELECT ID FROM Rezepte WHERE Name = ?",(Rname,))
            for row in Zspeicher:
                CocktailID = row[0]
            #print(CocktailID)
            c.execute("DELETE FROM Zusammen WHERE Rezept_ID = ?",(CocktailID,))
            ...
            Maker_List_null(w, DB, c)
            Rezepte_clear(w, DB, c, False)
            print("Rezept mit der ID: <" + str(CocktailID) + "> und dem Namen: <" + Rname + "> gelöscht!")
            standartbox("Rezept mit der ID: <" + str(CocktailID) + "> und dem Namen: <" + Rname + "> gelöscht!")
    else:
...
def save_Rezepte(w, DB, c):
    if w.LEpw.text() == globals.masterpassword:
        with open('Rezepte_export.csv', mode='a', newline='') as writer_file:
            ...
            csv_writer.writerow(["----- Neuer Export von %s -----" % datetime.date.today()])
            row1 = []
            row2 = []
            Zspeicher = c.execute("SELECT Name, Anzahl FROM Rezepte WHERE Anzahl > 0 ORDER BY Anzahl DESC, Name ASC")
            for row in Zspeicher:
                row1.append(row[0])
                row2.append(row[1])
            ...
            row1 = []
            row2 = []
            Zspeicher = c.execute("SELECT Name, Anzahl_Lifetime FROM Rezepte WHERE Anzahl_Lifetime > 0 ORDER BY Anzahl_Lifetime DESC, Name ASC")
            for row in Zspeicher:
                row1.append(row[0])
                row2.append(row[1])
            csv_writer.writerow(row1)
        ...
        DB.commit()
        standartbox("Alle Daten wurden exportiert und die zurücksetzbare Rezeptanzahl zurückgesetzt!")
    else:
        print("Falsches Passwort!")
        standartbox("Falsches Passwort!")
    w.LEpw.clear()
