...
class TestConfigManagerGetConfig:
    ...
    def test_get_config_serializes_values_correctly(self) -> None:
        ...
        # Check for pump config serialization
        assert all(isinstance(pump, dict) for pump in result["PUMP_CONFIG"])
...
class TestConfigManagerSetConfig:
    ...
    def test_set_config_without_validation_ignores_errors_and_keeps_old_value(self) -> None:
    ...
    def test_set_config_processes_non_list_before_list_success(self) -> None:
        """Test that non-list configs are processed before list configs when validation succeeds.

        This is important because list lengths might depend on other config values.
        """
        config = ConfigManager()
        # Set number of bottles to 1, and PUMP_CONFIG with 1 pump - should succeed
        config.set_config(
            {
                "MAKER_NUMBER_BOTTLES": 1,
                "PUMP_CONFIG": [{"pin": 10, "volume_flow": 20.5, "tube_volume": 5}],
            },
            validate=True,
        )
        assert config.MAKER_NUMBER_BOTTLES == 1
        assert len(config.PUMP_CONFIG) == 1

    def test_set_config_processes_non_list_before_list_failure(self) -> None:
        """Test that validation fails when PUMP_CONFIG has fewer elements than bottles.

        This ensures non-list configs are processed before list configs so validation
        uses the updated value of MAKER_NUMBER_BOTTLES.
        """
        config = ConfigManager()
        # Set number of bottles to 2, but PUMP_CONFIG with only 1 pump - should fail
        with pytest.raises(ConfigError):
            config.set_config(
                {
...
class TestConfigManagerSyncConfigToFile:
    ...
    def test_sync_config_to_file_writes_valid_yaml(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_sync_config_to_file_preserves_all_configs(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestConfigManagerAddConfig:
    ...
    def test_add_config_string(self) -> None:
        ...
        assert getattr(config, "TEST_STRING") == "default_value"
    ...
    def test_add_config_int(self) -> None:
        ...
        assert getattr(config, "TEST_INT") == 42
    ...
    def test_add_config_empty_list_with_type(self) -> None:
        ...
        assert getattr(config, "TEST_EMPTY_LIST") == []
    ...
    def test_add_config_preserves_existing_value(self) -> None:
        ...
        """
        config = ConfigManager()
        config.EXISTING_CONFIG = "existing_value"  # type: ignore
        config.add_config("EXISTING_CONFIG", "new_default")
        assert config.EXISTING_CONFIG == "existing_value"  # type: ignore
...
class TestConfigManagerAddSelectionConfig:
    ...
    def test_add_selection_config_with_default(self) -> None:
        ...
        assert getattr(config, "TEST_SELECT") == "opt2"
    ...
    def test_add_selection_config_uses_first_as_default(self) -> None:
        ...
        assert getattr(config, "TEST_SELECT") == "opt1"
    ...
    def test_add_selection_config_with_validator(self) -> None:
        ...
        config = ConfigManager()
        config.add_selection_config("TEST_SELECT", ["production", "test_mode"], validation_function=[no_test_validator])
        # Valid value
...
class TestIntegrationConfigDumpAndLoad:
    ...
    def test_dump_and_load_pump_config(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        config1 = ConfigManager()
        config1.set_config({"MAKER_NUMBER_BOTTLES": 2}, validate=True)
        config1.set_config(
            {
                "PUMP_CONFIG": [
                    {"pin": 10, "volume_flow": 20.5, "tube_volume": 5},
                    {"pin": 11, "volume_flow": 25.0, "tube_volume": 6},
                ]
            },
            validate=True,
        )
    ...
    def test_config_handles_corrupted_values(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestEdgeCasesConfigManager:
    ...
    def test_config_preserves_type_through_serialization(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
