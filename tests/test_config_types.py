...
class TestDictType:
    ...
    def test_validation_success(self) -> None:
        ...
        dict_types = {"name": StringType(), "age": IntType()}
        dict_type = DictType(dict_types, PumpConfig)
        dict_type.validate("test_config", {"name": "John", "age": 30})

    def test_validation_fails_for_non_dict(self) -> None:
        """Test that non-dict values fail validation."""
        dict_types = {"name": StringType()}
        dict_type = DictType(dict_types, PumpConfig)
        with pytest.raises(ConfigError, match="is not of type"):
            dict_type.validate("test_config", "not_a_dict")
    ...
    def test_serialization(self) -> None:
        ...
        dict_types = {"pin": IntType(), "volume_flow": FloatType(), "tube_volume": IntType()}
