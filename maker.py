...
def Rezepte_a_M(w, DB, c):
    ...
    for row in V_Rezepte:
        ...
        Zspeicher = c.execute("SELECT Zutaten_ID FROM Zusammen WHERE Rezept_ID = ?",(row,))
        for Werte in Zspeicher:
...
def Maker_Rezepte_click(w, DB, c):
    ...
    c.execute("SELECT Alkoholgehalt FROM Rezepte WHERE Name = ?",(w.LWMaker.currentItem().text(),))
    ...
    c.execute("SELECT Menge FROM Rezepte WHERE Name = ?",(w.LWMaker.currentItem().text(),))
    Zspeicher = c.fetchone()[0]
...
def Maker_Zubereiten(w, DB, c, normalcheck, devenvironment):
    ...
    if globals.startcheck == False:
        ...
        createcheck = True
        ##################################################################################
        # hier werden die Pins definiert #################################################
        ##################################################################################
        # Pinvektor = [14, 15, 18, 23, 24, 25, 8, 7, 17, 27, 22, 20]
        Pinvektor = globals.usedpins
        ##################################################################################
        # hier wird Volumenstrom definiert  (ml/s) #######################################
        ##################################################################################
        # Volumenstrom = [30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30]
        ...
        if not w.LWMaker.selectedItems():
            ...
            Zspeicher = c.execute("SELECT ID, Menge FROM Rezepte WHERE Name = ?",(w.LWMaker.currentItem().text(),))
            for row in Zspeicher:
                CocktailID = row[0]
                Cocktailmenge = row[1]
            ...
            Zspeicher = c.execute("SELECT Zusammen.Menge, Belegung.Flasche, Zusammen.Alkoholisch From Zusammen INNER JOIN Belegung ON Zusammen.Zutaten_ID = Belegung.ID WHERE Zusammen.Rezept_ID = ?",(CocktailID,))
            for row in Zspeicher:
                if row[2] == 1:
            ...
            for x in range(0,len(V_ZM)):
                c.execute("SELECT COUNT(*) FROM Belegung WHERE Flasche = ? AND Mengenlevel<?",(V_FNr[x], V_ZM[x]))
                Mengentest = c.fetchone()[0]
                if Mengentest != 0:
                    createcheck = False
                    c.execute("SELECT Zutaten.Name FROM Zutaten INNER JOIN Belegung ON Zutaten.ID = Belegung.ID WHERE Belegung.Flasche = ?", (V_FNr[x],))
                    mangelzutat = c.fetchone()[0]
                    print("Es ist in Flasche %i mit der Zutat %s nicht mehr genug Volumen vorhanden, %.0f ml wird benötigt!" %(V_FNr[x], mangelzutat, V_ZM[x]))
                    standartbox("Es ist in Flasche %i mit der Zutat %s nicht mehr genug Volumen vorhanden, %.0f ml wird benötigt!" %(V_FNr[x], mangelzutat, V_ZM[x]))
            ...
            if createcheck == True:
                ...
                for row in range(0, len(V_FNr)):
                    ...
                    print("Pin: " + str(Pinvektor[V_FNr[row] - 1]) + " wurde initialisiert" )
                ...
                while (T_aktuell < T_max and globals.loopcheck):
                    if (T_aktuell*100) % 10 == 0:
                        print(str(T_aktuell) + " von " + str(T_max) + " Sekunden ")
                    w.progressBar.setValue(T_aktuell/T_max*100)
                    for row in range(0, len(V_FNr)):
                        if V_Zeit[row]>T_aktuell:
                            if (T_aktuell*100) % 10 == 0:
                                print("Pin: " + str(Pinvektor[V_FNr[row] - 1]) + " aktiv, aktuelles Volumen: " + str(round(Volumenstrom[V_FNr[row] - 1]*T_aktuell, 1)))
                            if not devenvironment:
                                GPIO.output(Pinvektor[V_FNr[row] - 1], 0)
                            V_Verbrauch[row] += Volumenstrom[V_FNr[row] - 1]*0.01
                        else:
                            if (T_aktuell*100) % 10 == 0:
                                print("Pin: " + str(Pinvektor[V_FNr[row] - 1]) + " geschlossen!")
                            if not devenvironment:
                ...
                for row in range(0, len(V_FNr)):
                    print("Pin: " + str(Pinvektor[V_FNr[row] - 1]) + " wurde geschlossen")
                    if not devenvironment:
                        GPIO.output(Pinvektor[V_FNr[row] - 1], 1)
                #Wendet den Verbrauch auf die DB an
                for x in range(0,len(V_FNr)):
                    c.execute("UPDATE OR IGNORE Belegung SET Mengenlevel = Mengenlevel - ? WHERE Flasche = ?", (round(V_Verbrauch[x]), V_FNr[x]))
                ...
                if globals.loopcheck == False:
                    abbruchstring = " - Rezept wurde bei " + str(round(T_aktuell, 1)) + " s abgebrochen - " + str(round(Cocktailmenge*(T_aktuell + 0.01)/T_max)) + " ml"
                else:
                    abbruchstring = ""
                template = "{:8} | {}{}"
                ...
                for x in range(0,len(V_Verbrauch)):
                    c.execute("UPDATE OR IGNORE Zutaten SET Verbrauchsmenge = Verbrauchsmenge + ?, Verbrauch = Verbrauch + ? WHERE ID = (SELECT ID FROM Belegung WHERE Flasche = ?)", (round(V_Verbrauch[x]), round(V_Verbrauch[x]), V_FNr[x]))
                DB.commit()
                Belegung_progressbar(w, DB, c)
                w.progressBar.setValue(0)
                if globals.loopcheck:
                    standartbox("Der Cocktail ist fertig! Bitte kurz warten, falls noch etwas nachtropft.")
