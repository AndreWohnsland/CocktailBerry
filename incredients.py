...
def Zutat_eintragen(w, DB, c):
    ...
    c.execute("SELECT COUNT(*) FROM Zutaten WHERE Name=?",(w.LEZutatRezept.text(),))
        ...
        if Zutatentest == 0:
            try:
                ...
                if int(w.LEGehaltRezept.text()) <= 100:
                    c.execute("INSERT OR IGNORE INTO Zutaten(Name,Alkoholgehalt,Flaschenvolumen,Verbrauchsmenge,Verbrauch) VALUES (?,?,?,0,0)", (w.LEZutatRezept.text(), int(w.LEGehaltRezept.text()), int(w.LEFlaschenvolumen.text())))
                    DB.commit()
...
def Zutaten_delete(w, DB, c):
    ...
    if w.LEpw2.text() == globals.masterpassword:
        if not w.LWZutaten.selectedItems():
            ...
            Zspeicher = c.execute("SELECT ID FROM Zutaten WHERE Name = ?",(Zname,))
            ...
            if Zutatentest == 0:
                ...
                if Zutatentest == 0:
                    ...
                    # Belegung_einlesen(w, DB, c)
                    print("Zutat mit der ID: <" + str(ZID) + "> und dem Namen: <" + Zname + "> gelöscht!")
                    standartbox("Zutat mit der ID: <" + str(ZID) + "> und dem Namen: <" + Zname + "> gelöscht!")
                else:
                ...
                print("Zutat kann nicht gelöscht werden, da sie in " + str(Zutatentest) + " Rezept(en) genutzt wird!")
                standartbox("Zutat kann nicht gelöscht werden, da sie in " + str(Zutatentest) + " Rezept(en) genutzt wird!")
    else:
...
def Zutaten_Zutaten_click(w, DB, c):
    ...
    Zspeicher = c.execute("SELECT Alkoholgehalt, Flaschenvolumen FROM Zutaten WHERE Name = ?",(w.LWZutaten.currentItem().text(),))
    for row in Zspeicher:
    ...
    if (w.LEZutatRezept.text() == "") or (w.LEGehaltRezept.text() == "") or (w.LEFlaschenvolumen.text() == ""):
        ...
        try:
            ...
            if int(w.LEGehaltRezept.text()) <= 100:
                ...
                Zspeicher = c.execute("SELECT ID FROM Zutaten WHERE Name = ?",(altername,))
                for row in Zspeicher:
                    ZID = int(row[0])
                #print(neuerName,int(w.LEGehaltRezept.text()),ZID)
                c.execute("UPDATE OR IGNORE Zutaten SET Name = ?, Alkoholgehalt = ?, Flaschenvolumen = ? WHERE ID = ?",(neuerName,int(w.LEGehaltRezept.text()),int(w.LEFlaschenvolumen.text()),int(ZID)))
                DB.commit() 
                w.LEZutatRezept.clear()
                w.LEGehaltRezept.clear()
                w.LEFlaschenvolumen.clear()
                ...
                print("Alkoholgehalt und Flaschenvolumen kann nicht größer als 100 sein!")
                standartbox("Alkoholgehalt und Flaschenvolumen kann nicht größer als 100 sein!")
        except ValueError:
...
def Zutaten_Flvolumen_pm(w, DB, c, operator):
    ...
    if w.LEFlaschenvolumen.text() == "":
        ...
        if operator == "+":
            w.LEFlaschenvolumen.setText(str(int(w.LEFlaschenvolumen.text()) + 25))
...
def save_Zutaten(w, DB, c):
    if w.LEpw2.text() == globals.masterpassword:
        with open('Zutaten_export.csv', mode='a', newline='') as writer_file:
            ...
            row1 = []
            row2 = []
            Zspeicher = c.execute("SELECT Name, Verbrauch FROM Zutaten ORDER BY Verbrauch DESC, Name ASC")
            for row in Zspeicher:
                row1.append(row[0])
                row2.append(row[1])
            ...
            row1 = []
            row2 = []
            Zspeicher = c.execute("SELECT Name, Verbrauchsmenge FROM Zutaten ORDER BY Verbrauchsmenge DESC, Name ASC")
            for row in Zspeicher:
                row1.append(row[0])
                row2.append(row[1])
            csv_writer.writerow(row1)
        ...
        DB.commit()
        standartbox("Alle Daten wurden exportiert und die zurücksetzbaren Zutatenmengen zurückgesetzt!")
    else:
        print("Falsches Passwort!")
        standartbox("Falsches Passwort!")
    w.LEpw2.clear()