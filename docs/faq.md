...
Also, there is plenty of information at the [setup section](setup.md).
...
If the microservice is active and the connection is temporarily lost, the data is saved until the internet is back again.

### How to Wire the System

Check out [this section](hardware.md#possible-basic-circuit), this explains a basic circuit you could use.
...
You can look at the [hardware section](hardware.md) for some recommended displays.
...
The values will also be validated before a change is applied.
...
CocktailBerry is software and therefore will work on any machine, even without any parts of a cocktail machine.
...
To remove restricted mode you will have to click a specific combination of the different tabs.
...
If you click another tab in between, the counter will reset.

### What is the Available Button

If you click the available button under bottles, you get to another window, where you can select ingredients.
...
Relay arrays can have up to 8 switches controlled with one 5V input.
...
Like with most programs, this will exit the currently open program.
You will then be on your desktop.

### I don't Need a Password

Just leave the password empty / delete all numbers.
If the password setting is empty or zero, actions requiring a password will automatically succeed without prompting for a password.
...
When applying a new bottle, CocktailBerry will ask if it should also pump that much volume up.
Decline the prompt if the tube is still filled, for example when a peristaltic pump keeps the liquid in the tube.
...
This does not include some default LEDs used for general lighting of the machine, because they usually don't need to be controlled.
...
Identical logs are only shown once, with their latest occurrence time, as well as the count.
