...
- [ ] Give the user the possibility to backup and restore CocktailBerry data
  - [ ] Option in the UI to import/export data
  - [ ] Prompt for user to choose folder for backup
  - [ ] Generate own folder with db (and config) file
  - [ ] *Optional:* possibility only to export one of both?
  - [ ] Method / Class to overwrite existing files with backup ones
    - [ ] Think of also deleting previously stored files in the backup if overwrite
- [ ] Also Send Recipe Data (Ingredients) as Object to the Endpoint. Let the user handle the data.
