...
def Belegung_eintragen(w, DB, c):
    ...
    if dummy2 == 0:	
        for Flaschen_C in range(1, 11):
            ...
            Speicher_ID2 = 0
            CBBname = getattr(w, "CBB" + str(Flaschen_C))
            c.execute("UPDATE OR IGNORE Belegung SET Zutat_F = ? WHERE Flasche = ?",(CBBname.currentText(),Flaschen_C))
            Speicher_ID = c.execute("SELECT ID FROM Zutaten WHERE Name = ?", (CBBname.currentText(),))
            for row in Speicher_ID:
                Speicher_ID2 = row[0]
            c.execute("UPDATE OR IGNORE Belegung SET ID = ? WHERE Flasche = ?",(int(Speicher_ID2),Flaschen_C))
            DB.commit()
            #print(CBBname.currentText())
        Belegung_a(w, DB, c)
        Rezepte_a_M(w, DB, c)
...
def Belegung_einlesen(w, DB, c):
    ...
    for Flaschen_C in range(1, 11):
        CBBname = getattr(w, "CBB" + str(Flaschen_C))
        Testbelegung = c.execute("SELECT Zutat_F FROM Belegung WHERE Flasche=?",(Flaschen_C,))
        #print(Flaschen_C)
...
def Belegung_Flanwenden(w, DB, c):
    ...
    for Flaschen_C in range(1, 11):
        ...
        if PBname.isChecked():
            storevar = c.execute("SELECT Zutaten.Flaschenvolumen FROM Belegung INNER JOIN Zutaten ON Zutaten.ID = Belegung.ID WHERE Belegung.Flasche = ?",(Flaschen_C,))
            for row in storevar:
                storevol = row[0]
                #print(storevol)
            c.execute("UPDATE OR IGNORE Belegung SET Mengenlevel = ? WHERE Flasche = ?",(storevol,Flaschen_C))
    DB.commit()
    for Flaschen_C in range(1, 11):
        PBname = getattr(w, "PBneu" + str(Flaschen_C))
...
def Belegung_progressbar(w, DB, c):
    ...
    for x in range(1,11):
        ...
        storeval2 = c.execute("SELECT Zutaten.Flaschenvolumen FROM Belegung INNER JOIN Zutaten ON Zutaten.ID = Belegung.ID WHERE Belegung.Flasche = ?",(x,))
...
def CleanMachine(w, DB, c, devenvironment):
    ...
    if w.LECleanMachine.text() == globals.masterpassword:
        ...
        standartbox("Achtung!: Maschine wird gereinigt, genug Wasser bereitstellen!")
