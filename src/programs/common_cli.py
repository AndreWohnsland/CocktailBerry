...
def register_common_commands(cli: typer.Typer) -> None:  # noqa: C901, PLR0915
    ...
    def create_dispenser(dispenser_name: str) -> None:
        ...
        and stripped of special characters.
        """
        generate_skeleton_for("dispenser", dispenser_name)

    @cli.command()
    def create_hardware(hardware_name: str) -> None:
        """Create the base file for a custom hardware extension under the given name.

        The file is saved under the addons/hardware folder.
        File name will be the name converted to lower case, spaces are replaced with underscores
        and stripped of special characters.
        """
    ...
    @cli.command()
    def create_rfid(rfid_name: str) -> None:
        """Create the base file for a custom RFID extension under the given name.

        The file is saved under the addons/rfid folder.
        File name will be the name converted to lower case, spaces are replaced with underscores
        and stripped of special characters.
        """
        generate_skeleton_for("rfid", rfid_name)

    @cli.command()
    def create_led(led_name: str) -> None:
        """Create the base file for a custom LED extension under the given name.

        The file is saved under the addons/leds folder.
        File name will be the name converted to lower case, spaces are replaced with underscores
        and stripped of special characters.
        """
        generate_skeleton_for("led", led_name)
